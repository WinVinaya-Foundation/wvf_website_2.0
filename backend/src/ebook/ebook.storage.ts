...
export const EBOOK_UPLOADS_DIR = path.join(process.cwd(), 'uploads', 'ebooks');

const storage = multer.diskStorage({
  destination: (_req, _file, cb) => {
    if (!fs.existsSync(EBOOK_UPLOADS_DIR)) {
      fs.mkdirSync(EBOOK_UPLOADS_DIR, { recursive: true });
    }
    cb(null, EBOOK_UPLOADS_DIR);
  },
