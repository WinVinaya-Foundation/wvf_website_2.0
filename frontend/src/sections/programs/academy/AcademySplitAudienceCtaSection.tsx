...
import { academyClosingCtaContent } from '../../../pages/programs/academyContent';

const AUDIENCE_ICONS = [SchoolRoundedIcon, VolunteerActivismRoundedIcon, AccountBalanceRoundedIcon];

export default function AcademySplitAudienceCtaSection() {
  const { forCandidates, forDonors, forInstitutions } = academyClosingCtaContent;
  const cards = [
    { ...forCandidates, icon: AUDIENCE_ICONS[0] },
    { ...forDonors, icon: AUDIENCE_ICONS[1] },
    { ...forInstitutions, icon: AUDIENCE_ICONS[2] },
  ];

  return (
    <SectionContainer bgcolor="background.default" labelledBy="academy-split-cta-heading">
      <Box
        id="academy-split-cta-heading"
        sx={{
          position: 'absolute',
          width: 1,
          height: 1,
