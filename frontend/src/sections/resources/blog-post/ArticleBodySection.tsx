          ...
          if (block.type === 'quote') {
            return (
              ...
              >
                ...
                {block.attribution && (
                  <Typography sx={{ mt: 1.5, fontSize: '0.9rem', fontWeight: 700, color: 'primary.dark' }}>— {block.attribution}</Typography>
                )}
              </Box>
            );
          }
              ...
              <Box key={key} component="ul" sx={{ m: 0, my: 3, pl: 0, listStyle: 'none' }}>
                {block.items.map((item) => (
                  ...
                  >
                    ...
                    />
                    <Typography sx={{ fontSize: '1.05rem', lineHeight: 1.75, color: 'text.secondary' }}>{item}</Typography>
                  </Box>
                ))}
              </Box>
            );
            ...
            return (
              <Box key={key} component="ol" sx={{ m: 0, my: 3, pl: 0, listStyle: 'none', counterReset: 'blog-ordered-list' }}>
                {block.items.map((item) => (
                  ...
                  >
                    <Typography sx={{ fontSize: '1.05rem', lineHeight: 1.75, color: 'text.secondary', pt: '1px' }}>{item}</Typography>
                  </Box>
                ))}
              </Box>
            );
